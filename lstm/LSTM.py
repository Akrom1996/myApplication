...
# load the dataset
dataframe = pandas.read_csv('export_dataframe_nodeA.csv', usecols=[1], engine='python', skipfooter=3)
dataset = dataframe.values
dataset = dataset.astype('float32')

lengthTrain = int(len(dataset) * 0.9)
lengthValidation = len(dataset) - lengthTrain
train, test = dataset[:lengthTrain,:], dataset[lengthTrain:,:]
